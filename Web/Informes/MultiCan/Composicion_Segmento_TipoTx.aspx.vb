...
Public Class Composicion_Segmento_TipoTx
    ...
    Public Function RetornaSeleccionadas(ByVal Lstx As String, ByVal dde As ASPxDropDownEdit) As String
        ...
        Return LstID
    End Function
    Protected Sub CreaPie(TipoValor)
        ...
        Dim i As Integer
        'wchx_barras
        _ds = Multicanal.Composicion_Segmento_TipoTx(cbbx_anoini.SelectedItem.Value, cbbx_anofin.SelectedItem.Value, cbbx_mesini.SelectedItem.Value, cbbx_mesfin.SelectedItem.Value, IDSegmento, IDTransaccion)

        Grafico.Series.Clear()

        Grafico.DataSource = _ds.Tables(0)

        Grafico.Series.Add(New Series("CANAL", ViewType.Pie3D))
        Grafico.Series("CANAL").ArgumentDataMember = "CANAL"
        Grafico.Series("CANAL").ValueDataMembers.AddRange(New String() {TipoValor})
        Grafico.Series("CANAL").LabelsVisibility = DefaultBoolean.True
        Grafico.Series("CANAL").Label.TextPattern = "{A}: {VP:p1}"
        Grafico.Series("CANAL").Label.Border.Visibility = DefaultBoolean.False
        Grafico.Series("CANAL").Label.BackColor = Color.Transparent
        Grafico.Series("CANAL").LegendTextPattern = "{A}"

        Grafico.Legend.Visibility = False

        Dim LabelPie As PieSeriesLabel = CType(Grafico.Series(0).Label, PieSeriesLabel)
        LabelPie.Position = PieSeriesLabelPosition.TwoColumns
        LabelPie.ResolveOverlappingMode = ResolveOverlappingMode.Default

        Grafico.BorderOptions.Visibility = DefaultBoolean.False
    ...
    Protected Sub wchx_pie_CustomCallback(sender As Object, e As CustomCallbackEventArgs) Handles wchx_pie.CustomCallback
        If rdbx_valor.SelectedItem.Value = "Valor" Then
            CreaPie("MONTO")
        Else
            CreaPie("CANTIDAD")
        End If
